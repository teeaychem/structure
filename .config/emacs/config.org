...
(defmacro make-consult-filter (name extensions mode)
  `(progn
     ...
     (setq consult-buffer-filter (append consult-buffer-filter ,(intern-soft (format "+consult-%s-filter" name))))
     (defvar ,(intern (format "+consult-source-%s" name))
       `(:name      ,(format "%s" ',name)
         ...
         :items     ,(lambda () (consult--buffer-query
                                 :sort 'visibility
                                 :as #'buffer-name
                                 :exclude '() ;; (remq ',(intern-soft (format "+consult-%1$s-filter" name)) consult-buffer-filter)
                                 :mode ',mode)))
