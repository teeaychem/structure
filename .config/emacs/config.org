...
#+begin_src emacs-lisp
(defmacro make-consult-filter (name extension mode)
  `(progn
     (with-eval-after-load 'consult
     (defvar ,(intern (format "+consult-%1$s-filter" name)) ,(format ".\\.%1$s$" extension))
     (add-to-list 'consult-buffer-filter ,(intern-soft (format "+consult-%1$s-filter" name)))
     (defvar ,(intern (format "+consult-source-%1$s" name))
       `(:name      ,(format "%s" ',name)
         :narrow    ?x
         :category  buffer
         :face      consult-buffer
  ...
  ,(format "%s buffer source." name))
     (push ',(intern-soft (format "+consult-source-%1$s" name)) consult-buffer-sources)
     (print ,(intern-soft (format "+consult-source-%1$s" name)))
     )))


(make-consult-filter lua lua lua-mode)
(make-consult-filter python py (python-mode python-ts-mode))
;; (print (symbol-value (intern-soft (format "+consult-lua-filter"))))

;; (print +consult-source-lua)
;; (print +consult-source-python)
;; (print consult-buffer-sources)
#+end_src
