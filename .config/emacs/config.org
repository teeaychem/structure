...
In-progress macro to split consult by filename

#+begin_src emacs-lisp
(with-eval-after-load 'consult
  (progn
    ;; macro after load as used consult stuff, progn to sequence use of macro
    (defmacro make-consult-filter (name extensions mode)
      `(progn
         (with-eval-after-load 'consult
           (defvar ,(intern (format "+consult-%s-filter" name)) ',(mapcar (lambda (x) (format ".\\.%s$" x)) extensions))
           (setq consult-buffer-filter (append ,(intern-soft (format "+consult-%s-filter" name)) consult-buffer-filter))
           (defvar ,(intern (format "+consult-source-%s" name))
             `(:name      ,(format "%s" ',name)
                          :narrow    ?x
                          :category  buffer
                          :face      consult-buffer
                          ...
                          :items     ,(lambda () (consult--buffer-query
                                                  :sort 'visibility
                                                  :as #'buffer-name
                                                  :exclude ',(cl-set-difference consult-buffer-filter ',(intern-soft (format "+consult-%1$s-filter" name)))
                                                  :mode ',mode)))
             ,(format "%s buffer source." name))
           (push ',(intern-soft (format "+consult-source-%s" name)) consult-buffer-sources))))

    ;; instances of macro
    (make-consult-filter lua ("lua") lua-mode)
    (make-consult-filter python ("py") (python-mode python-ts-mode))
    (make-consult-filter c++ ("c" "cpp" "h") (c-mode c++-mode c-ts-mode c++-ts-mode))
    (make-consult-filter cmake ("cmake") (cmake-mode cmake-ts-mode))
    (make-consult-filter TeX ("tex") (latex-mode LaTeX-mode tex-mode TeX-mode))
