...
(defmacro make-consult-filter (name extensions mode)
  `(progn
     ...
     (defvar ,(intern (format "+consult-%1$s-filter" name)) ',(mapcar (lambda (x) (format ".\\.%1$s$" x)) extensions))
     (setq consult-buffer-filter (append consult-buffer-filter ,(intern-soft (format "+consult-%1$s-filter" name))))
